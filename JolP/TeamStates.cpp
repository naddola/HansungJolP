...
void Attacking::Enter(SoccerTeam* team)
{
  ...
  const int BlueRegions[TeamSize] = {3,45,37,32,22,25,10};
  const int RedRegions[TeamSize] = {52,37,39,31,25,16,10};
...
void Defending::Enter(SoccerTeam* team)
{
  ...
  const int BlueRegions[TeamSize] = {3,8,10,12,16,18,24};
  const int RedRegions[TeamSize] = {52,43,45,47,37,39,31};
  //set up the player's home regions
  if (team->Color() == SoccerTeam::blue) 
  {
    ChangePlayerHomeRegions(team, BlueRegions);
...
void PrepareForKickOff::Enter(SoccerTeam* team)
{
  ...
  team->SetPlayerClosestToBall2(NULL);
