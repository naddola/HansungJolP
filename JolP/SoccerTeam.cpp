...
#define THE_NUMBER_OF_PLAYIER 7
...
void SoccerTeam::CalculateClosestPlayerToBall()
{
	...
	for (it; it != m_Players.end(); ++it)
	{
		...
		{
			m_pPlayerClosestToBall2 = m_pPlayerClosestToBall;
			ClosestSoFar = dist;
...
void SoccerTeam::CreatePlayers()
{
	...
	//////////////////////////////////////////
	long	indicate;
	char name[THE_NUMBER_OF_PLAYIER][15];
	int		row = 0;
	///////////////////////////////////////////
	...
	if (Color() == blue)
	{
		rc=SQLExecDirect(hstmt, (SQLCHAR*)"SELECT PLAYER_NAME from PLAYERS where TEAM = 'Qpr' and STARTING = 1",SQL_NTS);
		while (TRUE) {
			rc = SQLFetch(hstmt);
			if (rc != SQL_SUCCESS && rc != SQL_SUCCESS_WITH_INFO) break;
			SQLGetData(hstmt, 1, SQL_C_CHAR, name[row], 15, &indicate);
			row++;
		}
	...
	{
		rc=SQLExecDirect(hstmt, (SQLCHAR*)"SELECT PLAYER_NAME from PLAYERS where TEAM = 'Bar' and STARTING = 1",SQL_NTS);
		while (TRUE) {
			rc = SQLFetch(hstmt);
			if (rc != SQL_SUCCESS && rc != SQL_SUCCESS_WITH_INFO) break;
			SQLGetData(hstmt, 1, SQL_C_CHAR, name[row], 15, &indicate);
